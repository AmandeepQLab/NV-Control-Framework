...
class ODMRPanel(QWidget):
    ...
    def __init__(self):
        ...
        self.trigger_delay_spin = self._add_double(
            delays_layout, "Camera Trigger Delay (s):", 0.0, 10.0, 4, 0.05
        )
        self.trigger_delay_spin.setToolTip(
            "Delay before the first camera trigger pulse.\n"
            "Recommended: 0.05 s.\n"
            "Reducing below 0.05 s caused ESR contrast to disappear."
        )

        self.frame_gap_spin = self._add_double(
            delays_layout, "Frame Gap (s):", 0.0, 10.0, 4, 0.05
        )
        self.frame_gap_spin.setToolTip(
            "Delay between OFF and ON camera frames inside one frequency point.\n"
            "Recommended: 0.05 s. Below 0.05 s was unstable for full-frame Andor readout."
        )
    ...
    def get_config(self):
        return {
            ...
            "frame_gap_s": self.frame_gap_spin.value(),
